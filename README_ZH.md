...
1. **有限的参数配置**：由于Dify平台的限制和实现选择，原论文中的某些超参数未完全配置：
   - 由于缺乏动态节点生成能力，无法在工作流中动态生成多个变体状态（n_p, n_q、n_a、n_r）
   - 由于缺乏嵌套循环支持，自我进化步骤（s_q、s_a、s_r）相比论文的实现有所简化
   - 没有重试机制来验证结构化JSON输出元素是否符合预期的模式
   - 虽然平台支持，但未配置代理参数（温度、top_k等）来探索多样化的搜索空间
